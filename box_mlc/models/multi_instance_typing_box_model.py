...
class MultiInstanceTypingBoxModel(Model):
    ...
    def __init__(
        self,
        vocab: Vocabulary,
            # TODO: copy over to vector model
    ...
    def get_scores(  # type:ignore
        ...
        **kwargs: Any,
        ...
        sentence_vec = self._encoder(
            sentences
        )  # shapes (batch, sentences, hidden_size)
        batch, hidden_dims = sentence_vec.shape
        ...
        if DISTANCE_MODE:
            ...
            dist_outside = identify closest corner of box to each datapoint and take L1 distance
            dist_inside = if point inside box, distance between point and box center, else distance between box and closest corner identified above
